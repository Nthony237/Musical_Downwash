...
def rotated_circle(angle, radius):
    """Circle corrected for lab axes: +X toward screen, +Y left."""
...
def apply_collision_avoidance(positions_list, min_dist=MIN_DRONE_DISTANCE):
    ...
    for frame in range(total):
        pts = [np.array([corr[d][0][frame],
                         corr[d][1][frame],
                         corr[d][2][frame]]) for d in range(n)]
        active = [i for i in range(n)
                  if not (pts[i][0] == 0 and pts[i][1] == 0
                          and pts[i][2] == 0)]
        if len(active) < 2:
            continue
        for _ in range(10):
...
def get_drone1_positions(features, duration=SONG_END, fps=30):
    ...
    for frame in range(total_frames):
        ...
        x_pos[frame]=x; y_pos[frame]=y; z_pos[frame]=z
...
def get_drone2_positions(features, duration=SONG_END, fps=30,
                         start_time=S1_END):
    total_frames = int(duration * fps)
...
def get_drone3_positions(features, duration=SONG_END, fps=30,
                         start_time=S2_END):
    total_frames = int(duration * fps)
    start_frame  = int(start_time * fps)
    rms_times  = features['rms_times']
    ...
    for frame in range(start_frame, total_frames):
        ...
        if t >= HEART_START:
            ...
            x = blend(x, FABRIC_CENTER_X, rise)
            y = blend(y, FABRIC_CENTER_Y, rise)
